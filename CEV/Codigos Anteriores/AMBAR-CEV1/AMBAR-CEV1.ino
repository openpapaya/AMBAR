...
void setup(void) {
  ...
  lcd.setCursor(0, 0);
    ...
    //
    //              ("      T    E    M    P      .   :         X    X    ,    X    X               ");
    //              ("  U   M    I    D    A     D    E    :    X    X    ,    X    X   %           ");
    // ***********************************************************************************************************
  lcd.print(" TEMP. :  XX,XX   ");
  lcd.print("UMIDADE : XX,XX   ");
}
void setup_wifi() {
  delay(10);
  Serial.println();
...
void reconnect() {
  while (!client.connected()) {
    ...
    client.publish("ambar/CEV/temp_reator1", "Conectando");
...
void loop() {
  ...
  if (now - lastTemp > 500) {
    ...
    Serial.println("ºC");
    dtostrf(temp[0], 1, 2, tempR1str);
    client.publish("ambar/CEV/temp_reator1", tempR1str);
    // Visor do LCD: 0x00 0x01 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09 0x0A 0x0B 0x0C 0x0D 0x0E 0x0F
    //               0x40 0x41 0x42 0x43 0x44 0x45 0x46 0x47 0x48 0x49 0x4A 0x4B 0x4C 0x4D 0x4E 0x4F
    lcd.setCursor(8, 0);  // Coluna, Linha
    lcd.print(tempR1str);
    dtostrf(temp[1], 1, 2, tempR2str);
    client.publish("ambar/CEV/temp_reator2", tempR2str);
